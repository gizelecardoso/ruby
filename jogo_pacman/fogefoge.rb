...
def encontra_jogador mapa
    ...
    mapa.each_with_index do |linha_atual, linha|
        coluna_do_heroi = linha_atual.index caracter_do_heroi #devolve a coluna ou devolve nil (nulo)
        if coluna_do_heroi
            jogador = Heroi.new
            jogador.linha = linha
...
def posicao_valida? mapa, posicao
    ...
    if estourou_linha || estourou_colunas
        return false
    end
...
def copia_mapa mapa
    novo_mapa = mapa.join("\n").tr("F", " ").split("\n") #join - junta tudo em uma unica string => tr - transforma todos os valore F em vazio => split - monta a matriz novamente pelo \n
end
